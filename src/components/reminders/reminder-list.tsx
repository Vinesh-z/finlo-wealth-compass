  ...
  return <Card className="h-full">
      ...
      <CardContent>
        <div className="space-y-6">
          {Object.keys(groupedReminders).length > 0 ? Object.entries(groupedReminders).map(([dateKey, dayReminders]) => <div key={dateKey}>
                <div className="flex items-center gap-2 mb-2">
                  <div className="h-1 w-1 rounded-full bg-primary"></div>
                  <h3 className="text-sm font-semibold">
                    {new Date(dateKey).toDateString() === new Date().toDateString() ? "Today" : new Date(dateKey).toDateString() === new Date(new Date().setDate(new Date().getDate() + 1)).toDateString() ? "Tomorrow" : formatDate(new Date(dateKey))}
                  </h3>
              ...
              return <Card key={reminder.id} className={cn("overflow-hidden border", overdueReminder ? "border-red-500" : reminder.isCompleted ? "bg-muted" : "")}>
                        <div className="p-4">
                          <div className="flex items-start gap-2">
                            <Checkbox checked={reminder.isCompleted} onCheckedChange={checked => handleToggleComplete(reminder.id, checked as boolean)} className="mt-1" />
